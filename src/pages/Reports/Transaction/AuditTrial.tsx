...
const AuditTrial: React.FC = () => {
  ...
  const [data, setData] = useState<AuditTrialData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const url = `${API_URL}/reports/audit-trial/`;
          ...
          <table className="min-w-full border border-gray-300">
            <thead className="bg-gray-200 dark:bg-gray-700">
              <tr>
                ...
                <th className="border p-2 text-left">By User</th>
              </tr>
            </thead>
            <tbody>
              {filteredData.map((row, index) => (
                <tr key={index} className="border dark:border-gray-600">
                  ...
                  <td className="border p-2 text-left">{row.voucher_no}</td>
                  <td className="border p-2 text-left">{row.action}</td>
                  <td className="border p-2 text-left">{row.by_user || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
