...
interface TCSReceivableResponse {
  transactions?: TCSReceivableData[];
}
...
const TCSReceivable: React.FC = () => {
  const [data, setData] = useState<TCSReceivableData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const url = `${API_URL}/reports/tcs-receivable/`;

        const response = await axios.get<TCSReceivableData[] | TCSReceivableResponse>(url, {
          headers: {
          ...
          <table className="min-w-full border border-gray-300">
            <thead className="bg-gray-200 dark:bg-gray-700">
              <tr>
                ...
                <th className="border p-2 text-left">TCS Amount</th>
            ...
            <tbody>
              {filteredData.map((row, index) => (
                <tr key={index} className="border dark:border-gray-600">
                  ...
                  <td className="border p-2 text-left">{row.taxableAmount}</td>
                  <td className="border p-2 text-left">{row.amount}</td>
                  <td className="border p-2 text-left">{row.tcsAmount}</td>
