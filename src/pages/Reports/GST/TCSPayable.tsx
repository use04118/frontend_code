...
const TCSPayable: React.FC = () => {
  const [data, setData] = useState<TCSPayableData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      ...
      try {
        ...
        const url = `${API_URL}/reports/tcs-payable/`;
            ...
            <tbody>
              {filteredData.map((row, index) => (
                <tr key={index} className="border dark:border-gray-600">
                  ...
                  <td className="border p-2 text-left">{Number(row.Taxable_amount).toFixed(2)}</td>
                  <td className="border p-2 text-left">{Number(row.amount).toFixed(2)}</td>
                  <td className="border p-2 text-left">{Number(row.Tcs_amount).toFixed(2)}</td>
