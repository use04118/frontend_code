...
const EditQuotationForm = () => {
  ...
  const [QuotationNo, setSalesQuotationNo] = useState(1);
  ...
  useEffect(() => {
    ...
    if (location.state?.quotationData) {
      ...
      if (location.state.quotationData.party) {
        fetchPartyDetails(location.state.quotationData.party);
      }
    } else {
      fetchQuotationDetails();
    }
  ...
  const fetchQuotationDetails = async () => {
    try {
      ...
      const quotationDate = data.date; // Assuming date is in 'YYYY-MM-DD' format from backend
      ...
      setSalesQuotationNo(parseInt(QuotationNO));
      ...
      const updatedDueDate = new Date(quotationDate);
      ...
      // Assuming the data includes 'quotation_items' array
      const quotationItems = data.quotation_items.map((item) => {
      ...
      setSelectedItems(quotationItems);
      // console.log(quotationItems); // Log the data to inspect
    } catch (error) {
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/sales/quotation/${id}/`, {
        ...
        body: JSON.stringify(quotationData), // Corrected data object
      });

      if (response.ok) {
        alert('Quotation updated successfully!');
        navigate(`/Sales/Sales-Quotation`);
      } else {
  ...
  // useEffect(() => {
  //   const savedQuotationNo = localStorage.getItem('QuotationNo');
  //   if (savedQuotationNo) {
  //     setSalesQuotationNo(parseInt(savedQuotationNo)); // Increment by 1
  //   }
  // }, []);

  const handleSubmit = async () => {
    if (!selectedParty) {
      alert('Please select a party before saving the quotation!');
      return;
    }
    ...
    const quotationData = {
      ...
      date: formatDate(quotationDate), // Format the date before sending
      payment_term: paymentTerms,
      // due_date:dueDate,
      quotation_items: formattedItems, // Array of items
      // taxableAmount,
  ...
  return (
    <>
      <Breadcrumb pageName="Sales Quotations" />
