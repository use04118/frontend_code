...
const CreateQuotationForm = () => {
  ...
  const [salesQuotationNo, setSalesQuotationNo] = useState(1);
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/sales/quotation/next-number/`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_quotation_number;
        setSalesQuotationNo(nextNumber);
        console.log('qy', salesQuotationNo);
        localStorage.setItem('salesQuotationNo', nextNumber.toString());
      }
    } catch (error) {
  ...
  const handleSaveQuotation = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      if (!token) throw new Error('Token not found');
      const response = await fetch(`${API_URL}/sales/quotation/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
