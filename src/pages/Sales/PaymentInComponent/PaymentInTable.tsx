...
const PaymentInTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPaymentIns = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/sales/paymentin/`, {
          headers: {
        ...
        if (Array.isArray(response.data.results)) {
          setSalePaymentInData(response.data.results);
        } else {
          console.error('Unexpected API response format:', response.data);
          setSalePaymentInData([]); // Default to empty array if data is not an array
        }

        setLoading(false);
      } catch (error) {
        console.error('Error fetching sales paymentIns:', error);
        setSalePaymentInData([]); // Ensure it's always an array
        setLoading(false);
      }
    };
  ...
  const filteredPaymentIn = salePaymentInData.filter((paymentIn) => {
    const paymentInDate = format(new Date(paymentIn.date), 'yyyy-MM-dd');

    const isWithinDateRange =
      startDate && endDate
        ? paymentInDate >= startDate && paymentInDate <= endDate
        : true;

    return (
      paymentIn.payment_in_number.toString().includes(searchTerm) &&
      isWithinDateRange
    );
  });
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold">PaymentIn Details</h2>
        ...
        <div className="relative w-1/3">
          ...
          <input
            type="text"
            placeholder="Search by PaymentIn Number..."
        ...
        {dateRange === 'Custom Date Range' && (
          <div className="flex gap-2">
            <DatePicker
              ...
              onChange={(date: Date | null) => setCustomStartDate(date)} // Allow null here
              className="p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              placeholderText="Start Date"
            />
            <DatePicker
              selected={customEndDate}
              onChange={(date: Date | null) => setCustomEndDate(date)} // Allow null here
              className="p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
        ...
        <table className="min-w-full border border-gray-300">
          <thead>
            <tr className="bg-gray-200 text-left dark:bg-gray-600 ">
              ...
              <th className="border px-4 py-2">PaymentIn Number</th>
          ...
          <tbody>
            {filteredPaymentIn.length > 0 ? (
              filteredPaymentIn.map((data) => (
                <tr
                  key={data.payment_in_number}
                  onClick={() => {
                    console.log('Navigating to:', data.id);
                    navigate(`/Payment-In-Details/${data.id}`);
                  }}
                  className="cursor-pointer hover:bg-gray-100"
                >
                  <td className="border px-4 py-2">{data.date}</td>
                  <td className="border px-4 py-2">{data.payment_in_number}</td>
