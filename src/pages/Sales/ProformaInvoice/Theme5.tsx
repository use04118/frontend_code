...
const Theme5 = () => {
  const { id } = useParams(); // Get proforma invoice number from URL
  ...
  const generatePDF = () => {
    setTimeout(() => {
      const element = document.getElementById('proforma-content');
      ...
      const options = {
        margin: [10, 10, 10, 10],
        filename: `${proformaInvoiceData.proforma_no}_ProformaInvoice.pdf`,
        html2canvas: {
          scale: 5,
  ...
  const handleEdit = () => {
    navigate(`/edit-proformainvoice/${id}`, { state: { proformaInvoiceData } });
  };
  ...
  const partyInfo = partyData[proformaInvoiceData?.party] || {
    name: 'Loading...',
    mobile: 'Loading...',
    address: 'Loading...',
  };
  ...
  const handleConvertToInvoice = async () => {
    const confirmConvert = window.confirm(
      'Are you sure you want to convert this proforma to an invoice?',
    );
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/sales/convert-proforma-to-invoice/${id}/`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        alert('Proforma successfully converted to invoice!');
        navigate('/Sales/Sales-Invoice'); // Redirect to invoices page
      } else {
        const errorData = await response.json();
  ...
  if (!proformaInvoiceData) return <p>Loading...</p>;
    ...
    <div className="bg-white dark:bg-gray-900 p-8">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="mt-2 text-right">
              <p style={{ color: themeColors.headerText }}>
                <strong>Proforma No.:</strong> {proformaInvoiceData.quotation_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Proforma Date:</strong> {proformaInvoiceData.date}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Due Date:</strong> {proformaInvoiceData.due_date}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <tbody>
            {proformaInvoiceData.proforma_items.map((item, index) => (
        ...
        >
          ...
          <p className="w-1/5 text-center">{proformaInvoiceData.proforma_items.length}</p>
          <p className="w-1/5 text-right">
            ₹{(proformaInvoiceData.total_amount - proformaInvoiceData.taxable_amount).toFixed(2)}
          </p>
          <p className="w-1/5 text-right">₹{proformaInvoiceData.total_amount}</p>
        </div>
        ...
        <div className="flex justify-between text-sm mb-4">
          ...
          >
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Taxable Amount:</p>
              <p>{proformaInvoiceData.taxable_amount}</p>
            </div>
            {/* Grouping CGST, SGST, and CESS Rates */}
            {(() => {
              const taxSummary = {};

              proformaInvoiceData.proforma_items.forEach((item) => {
            ...
            {isTcsEnabled && (
                <div className="flex justify-between mb-1">
                  ...
                  <p className="text-sm font-bold">
                    ₹{proformaInvoiceData.tcs_amount || '-'}
                  </p>
                </div>
              )}
              <div className="flex justify-between">
                ...
                <p className="text-sm font-bold">
                  ₹{proformaInvoiceData.total_amount}
                </p>
              </div>
            </div>
        ...
        >
          ...
          <p>{numberToWords(proformaInvoiceData.total_amount)} Rupees</p>
        </div>
      </div>
