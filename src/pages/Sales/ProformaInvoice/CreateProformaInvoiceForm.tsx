...
const CreateProformaInvoiceForm = () => {
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/sales/proforma/next-number/`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_proforma_number;
        setSalesProformaInvoiceNo(nextNumber);
        localStorage.setItem('proformaInvoiceNo', nextNumber.toString());
      }
    } catch (error) {
  ...
  const handleSaveProformaInvoice = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      if (!token) throw new Error('Token not found');
      const response = await fetch(`${API_URL}/sales/proforma/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        alert('ProformaInvoice Saved Successfully!');
        console.log(result);
        fetchNextInvoiceNumber();
  ...
  return (
    <>
      <Breadcrumb pageName="ProformaInvoices" />
      <div className="bg-gray-100 dark:bg-gray-900 p-4">
        <div className="max-w-7xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
          ...
          <div className="flex justify-start gap-4">
            ...
            <button
              onClick={handleSaveProformaInvoice}
