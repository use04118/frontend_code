...
const Theme3 = () => {
  ...
  useEffect(() => {
    ...
    const fetchCreditNoteDetails = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/sales/creditnote/${id}/`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchTcsSetting = async () => {
      try {
        ...
        console.error('Error fetching TCS/TDS settings:', error);
      }
    };
  ...
  const generatePDF = () => {
    setTimeout(() => {
      const element = document.getElementById('creditnote-content');
      const backgroundColor = themeColors.bodyBg;

      const options = {
        margin: [10, 10, 10, 10],
        filename: `${creditNoteData.credit_note_no}_CreditNote.pdf`,
        html2canvas: {
          scale: 5,
          backgroundColor,
          logging: false,
  ...
  if (!creditNoteData) return <p>Loading...</p>;

  const partyInfo = partyData[creditNoteData.party] || {
    name: 'Loading...',
    mobile: 'Loading...',
    address: 'Loading...',
    state: '',
  ...
  const handleEdit = () => {
    navigate(`/edit-creditnote/${id}`, { state: { creditNoteData } });
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to delete credit note: ${errorData.message}`);
      }
    } catch (error) {
    ...
    <div className="bg-white dark:bg-gray-900 p-8">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="mt-2 text-right">
              <p style={{ color: themeColors.headerText }}>
                <strong>CreditNote No.:</strong> {creditNoteData.credit_note_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>SalesReturn No.:</strong> {creditNoteData.salesreturn_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>CreditNote Date:</strong> {creditNoteData.date}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <tbody>
            {creditNoteData.creditnote_items.map((item, index) => (
        ...
        >
          ...
          <p className="w-1/5 text-center">{creditNoteData.creditnote_items.length}</p>
          <p className="w-1/5 text-right">
            ₹{(creditNoteData.total_amount - creditNoteData.taxable_amount).toFixed(2)}
          </p>
          <p className="w-1/5 text-right">₹{creditNoteData.total_amount}</p>
        </div>
        ...
        <div className="flex justify-between text-sm mb-4">
          ...
          >
            ...
            {(() => {
              ...
              creditNoteData.creditnote_items.forEach((item) => {
              ...
              {isTcsEnabled && (
                <div className="flex justify-between mb-1">
                  ...
                  <p className="text-sm font-bold">
                    ₹{creditNoteData.tcs_amount || '-'}
                  </p>
                </div>
              )}
              <div className="flex justify-between">
                <p className="text-sm font-bold">TOTAL AMOUNT</p>
                <p className="text-sm font-bold">₹{creditNoteData.total_amount}</p>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Received Amount:</p>
              <p>₹{creditNoteData.amount_received}</p>
            </div>
            <div className="flex justify-between items-center mb-2 text-right">
              <p className="text-sm font-bold">Balance</p>
              <p className="text-sm font-bold">₹{creditNoteData.balance_amount}</p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <p>{numberToWords(creditNoteData.total_amount)} Rupees</p>
        </div>
      </div>
