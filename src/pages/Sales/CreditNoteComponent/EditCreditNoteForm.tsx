...
const EditCreditNoteForm = () => {
  ...
  useEffect(() => {
    ...
    if (location.state?.creditNoteData) {
      setCreditNoteData(location.state.creditNoteData);
      if (location.state.creditNoteData.party) {
        fetchPartyDetails(location.state.creditNoteData.party);
      }
    } else {
      fetchCreditNoteDetails();
    }
  ...
  const fetchCreditNoteDetails = async () => {
    try {
      ...
      const creditNoteDate = data.date; // Assuming date is in 'YYYY-MM-DD' format from backend
      ...
      setCreditNoteNo(data.credit_note_no);
      ...
      const updatedDueDate = new Date(creditNoteDate);
      ...
      // Assuming the data includes 'creditNote_items' array
      const creditNoteItems = data.creditnote_items.map((item) => {
      ...
      setSelectedItems(creditNoteItems);
      console.log(creditNoteItems); // Log the data to inspect
    } catch (error) {
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/sales/creditnote/${id}/`, {
        ...
        body: JSON.stringify(creditNoteData), // Corrected data object
      });

      if (response.ok) {
        alert('CreditNote updated successfully!');
        navigate(`/Sales/Sales-CreditNote`);
      } else {
        alert('Failed to update creditNote.');
      }
    } catch (error) {
  ...
  // useEffect(() => {
  //   const savedCreditNoteNo = localStorage.getItem('creditNoteNo');
  //   if (savedCreditNoteNo) {
  //     setCreditNoteNo(parseInt(savedCreditNoteNo)); // Increment by 1
  //   }
  // }, []);

  const handleSubmit = async () => {
    if (!selectedParty) {
      alert('Please select a party before saving the creditNote!');
      return;
    }
    ...
    const creditNoteData = {
      ...
      date: formatDate(creditNoteDate),
          ...
          <div className="flex flex-col-reverse md:flex-row-reverse justify-between items-start gap-6 mb-12">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <input
                  type="text"
                  value={creditNoteData.credit_note_no}
                  readOnly
