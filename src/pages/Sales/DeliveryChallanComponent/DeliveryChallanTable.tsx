...
interface SaleDeliveryChallanData {
  date: string;
...
const DeliveryChallanTable: React.FC = () => {
  ...
  const [saleDeliveryChallanData, setSaleDeliveryChallanData] = useState<SaleDeliveryChallanData[]>([]);
  ...
  useEffect(() => {
    const fetchDeliveryChallans = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/sales/deliverychallan/`, {
          headers: {
        ...
        if (Array.isArray(data.results)) {
          setSaleDeliveryChallanData(data.results);
        } else {
          console.error("Unexpected API response format:", data);
          setSaleDeliveryChallanData([]); // Default to empty array if data is not an array
        }
        
        setLoading(false);
      } catch (error) {
        console.error('Error fetching sales deliverychallans:', error);
        setSaleDeliveryChallanData([]); // Ensure it's always an array
        setLoading(false);
      }
    };
    ...
    const fetchParties = async () => {
      try {
        ...
        if (Array.isArray(data.results)) {
          ...
          data.results.forEach((party: Party) => {
  ...
  const filteredDeliveryChallan = saleDeliveryChallanData.filter((deliveryChallan) => {
    const deliveryChallanDate = format(new Date(deliveryChallan.date), 'yyyy-MM-dd');

    const isWithinDateRange =
      startDate && endDate
        ? deliveryChallanDate >= startDate && deliveryChallanDate <= endDate
        : true;

    return (
      deliveryChallan.delivery_challan_no.toString().includes(searchTerm) &&
      isWithinDateRange
    );
  });
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold">DeliveryChallan Details</h2>
        ...
        <div className="relative w-1/3">
          ...
          <input
            type="text"
            placeholder="Search by DeliveryChallan Number..."
        ...
        <table className="min-w-full border border-gray-300">
          <thead>
            <tr className="bg-gray-200 text-left dark:bg-gray-600 ">
              ...
              <th className="border px-4 py-2">DeliveryChallan Number</th>
          ...
          <tbody>
            {filteredDeliveryChallan.length > 0 ? (
              filteredDeliveryChallan.map((data) => (
                <tr
                  key={data.delivery_challan_no}
                  onClick={() => {
                    console.log("Navigating to:", data.id);
                    navigate(`/theme4/${data.id}`);
                  }}
                  className="cursor-pointer hover:bg-gray-100"
                >
                  <td className="border px-4 py-2">{data.date}</td>
                  <td className="border px-4 py-2">{data.delivery_challan_no}</td>
