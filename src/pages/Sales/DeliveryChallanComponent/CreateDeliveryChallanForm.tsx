...
const CreateDeliveryChallanForm = () => {
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/sales/deliverychallan/next-number/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_deliverychallan_number;
        setSalesDeliveryChallanNo(nextNumber);
        localStorage.setItem('deliveryChallanNo', nextNumber.toString());
      }
    } catch (error) {
  ...
  const handleSaveDeliveryChallan = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      if (!token) throw new Error('Token not found');
      const response = await fetch(`${API_URL}/sales/deliverychallan/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        alert('DeliveryChallan Saved Successfully!');
        console.log(result);
        fetchNextInvoiceNumber();
  ...
  return (
    <>
      <Breadcrumb pageName="DeliveryChallans" />
