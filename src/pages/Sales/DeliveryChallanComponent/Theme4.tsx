...
const Theme4 = () => {
  ...
  useEffect(() => {
    ...
    const fetchDeliveryChallan = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/sales/deliverychallan/${id}/`, {
          headers: {
    ...
    const fetchParties = async () => {
      try {
        const token = localStorage.getItem('accessToken');
        const res = await fetch(`${API_URL}/parties/parties`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    const fetchTcsSetting = async () => {
      try {
        const token = localStorage.getItem('accessToken');
        const res = await fetch(`${API_URL}/sales/settings/tcs-tds/`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  const generatePDF = () => {
    setTimeout(() => {
      ...
      const options = {
        margin: [10, 10, 10, 10],
        filename: `${challanData.challan_no}_DeliveryChallan.pdf`,
        html2canvas: {
          scale: 5,
  ...
  const handleEdit = () => {
    navigate(`/edit-deliverychallan/${id}`, { state: { challanData } });
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Failed to delete challan: ${errorData.message}`);
      }
    } catch (error) {
  ...
  const handleConvertToInvoice = async () => {
    const confirmConvert = window.confirm(
      'Convert this delivery challan to an invoice?',
    );
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/sales/convert-deliverychallan-to-invoice/${id}/`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (res.ok) {
        alert('Challan converted to invoice!');
  ...
  const partyInfo = partyData[challanData?.party] || {
    name: 'Loading...',
    mobile: 'Loading...',
    address: 'Loading...',
  };
  ...
  const isSameState =
    partyInfo?.state?.trim().toLowerCase() ===
  ...
  if (!challanData) return <p>Loading...</p>;
    ...
    <div className="bg-white dark:bg-gray-900 p-8">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="mt-2 text-right">
              <p style={{ color: themeColors.headerText }}>
                <strong>DeliveryChallan No.:</strong> {challanData.delivery_challan_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>DeliveryChallan Date:</strong> {challanData.date}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Due Date:</strong> {challanData.date}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <tbody>
            {challanData.deliverychallan_items.map((item, index) => (
        ...
        >
          ...
          <p className="w-1/5 text-center">{challanData.deliverychallan_items.length}</p>
          <p className="w-1/5 text-right">
            ₹{(challanData.total_amount - challanData.taxable_amount).toFixed(2)}
          </p>
          <p className="w-1/5 text-right">₹{challanData.total_amount}</p>
        </div>
        ...
        <div className="flex justify-between text-sm mb-4">
          ...
          >
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Taxable Amount:</p>
              <p>{challanData.taxable_amount}</p>
            </div>
            {(() => {
              const taxSummary = {};

              challanData.deliverychallan_items.forEach((item) => {
            ...
            <div className="mb-4 border-t border-b pt-2 pb-2 text-right" style={{ borderColor: themeColors.tableBorder }}>
              {isTcsEnabled && (
                <div className="flex justify-between mb-1">
                  <p className="text-sm font-bold">TCS AMOUNT</p>
                  <p className="text-sm font-bold">
                    ₹{challanData.tcs_amount || '-'}
                  </p>
                </div>
              )}
              <div className="flex justify-between">
                <p className="text-sm font-bold">TOTAL AMOUNT</p>
                <p className="text-sm font-bold">₹{challanData.total_amount}</p>
              </div>
            </div>
        ...
        >
          ...
          <p>{numberToWords(challanData.total_amount)} Rupees</p>
        </div>
      </div>
