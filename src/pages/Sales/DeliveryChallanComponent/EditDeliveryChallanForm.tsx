...
const EditDeliveryChallanForm = () => {
  const API_URL = import.meta.env.VITE_API_URL;
  const navigate = useNavigate();
  const [DeliveryChallanNo, setSalesDeliveryChallanNo] = useState(1);
  ...
  useEffect(() => {
    ...
    if (location.state?.deliveryChallanData) {
      setDeliveryChallanData(location.state.deliveryChallanData);
      if (location.state.deliveryChallanData.party) {
        fetchPartyDetails(location.state.deliveryChallanData.party);
      }
    } else {
      fetchDeliveryChallanDetails();
    }
  ...
  const fetchDeliveryChallanDetails = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/sales/deliverychallan/${id}/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      const deliveryChallanDate = data.date; // Assuming date is in 'YYYY-MM-DD' format from backend
      ...
      setSalesDeliveryChallanNo(data.delivery_challan_no)
      setDeliveryChallanDate(deliveryChallanDate);
      ...
      setDeliveryChallanData((prevData) => ({
        ...prevData,
      }));

      // Assuming the data includes 'deliveryChallan_items' array
      const deliveryChallanItems = data.deliverychallan_items.map((item) => {
      ...
      setSelectedItems(deliveryChallanItems);
      console.log(deliveryChallanItems); // Log the data to inspect
    } catch (error) {
      console.error('Error fetching deliveryChallan details:', error);
    }
  };
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/sales/deliveryChallans/${id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify(deliveryChallanData), // Corrected data object
      });

      if (response.ok) {
        alert('DeliveryChallan updated successfully!');
        navigate(`/Sales/Sales-DeliveryChallan`);
      } else {
        alert('Failed to update deliveryChallan.');
      }
    } catch (error) {
      console.error('Error updating deliveryChallan:', error);
    } finally {
  ...
  // useEffect(() => {
  //   const savedDeliveryChallanNo = localStorage.getItem('DeliveryChallanNo');
  //   if (savedDeliveryChallanNo) {
  //     setSalesDeliveryChallanNo(parseInt(savedDeliveryChallanNo)); // Increment by 1
  //   }
  // }, []);

  const handleSubmit = async () => {
    if (!selectedParty) {
      alert('Please select a party before saving the deliveryChallan!');
      return;
    }
    ...
    const deliveryChallanData = {
      ...
      date: formatDate(deliveryChallanDate), // Format the date before sending

      // due_date:dueDate,
      deliverychallan_items: formattedItems, // Array of items
      // taxableAmount,
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Failed to save deliveryChallan');
      }
    } catch (error) {
      console.error('Error saving deliveryChallan:', error);
  ...
  return (
    <>
      <Breadcrumb pageName="Sales DeliveryChallans" />
        ...
        <div className="max-w-7xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
              ...
              <div className="flex items-center space-x-2">
                ...
                <DatePicker
                  ...
                  onChange={(date: Date) => {
                    ...
                    setDeliveryChallanDate(resetDate);
                  }}
                  className="border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-black dark:text-white p-2 rounded"
          ...
          <div className="flex justify-start gap-4">
            ...
            <button
              ...
              disabled={isSaving}
            >
              {isSaving ? 'Saving...' : 'Save DeliveryChallan'}
            </button>
          </div>
        </div>
      </div>
...
export default EditDeliveryChallanForm;
