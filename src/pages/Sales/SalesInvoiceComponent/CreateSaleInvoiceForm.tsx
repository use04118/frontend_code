...
const CreateSalesInvoiceForm: React.FC = () => {
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/sales/invoices/next-number/`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_invoice_number;
        setSalesInvoiceNo(nextNumber);
        localStorage.setItem('salesInvoiceNo', nextNumber.toString());
      }
    } catch (error) {
  ...
  const handleSaveInvoice = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(result);
        // Fetch the next invoice number after successful save
        fetchNextInvoiceNumber();
