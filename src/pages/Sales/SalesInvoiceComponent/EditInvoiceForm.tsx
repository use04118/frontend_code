...
const EditInvoiceForm = () => {
  ...
  const fetchInvoiceDetails = async () => {
    try {
      ...
      // Existing invoice setup...
      const invoiceDate = data.date;
      const paymentTerms = data.payment_term;
      const dueDate = data.due_date;
      const amountReceived = data.amount_received;
      setSalesInvoiceNo(data.invoice_no);
      ...
      updatedDueDate.setDate(updatedDueDate.getDate() + paymentTerms);
      ...
      setInvoiceData((prevData) => ({
        ...prevData,
        due_date: updatedDueDate.toISOString().split('T')[0],
      }));
  ...
  useEffect(() => {
    ...
    loadData();
  }, [id, tcsRates.length]); // ⛏️ Re-run if `tcsRates` load late
  ...
  const handleSubmit = async () => {
    ...
    const invoiceData = {
      ...
      date: formatDate(invoiceDate), // Format the date before sending
      ...
      if (response.ok) {
        ...
        /*  if (amountReceived > 0) {
          ...
          const paymentPayload = {
            ...
            remarks: `Received for Sales Invoice #${salesInvoiceNo}`,
