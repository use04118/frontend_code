...
const EditSalesReturnForm = () => {
  const API_URL = import.meta.env.VITE_API_URL;
  const navigate = useNavigate();
  const [salesReturnNo, setSalesReturnNo] = useState(1);
  ...
  useEffect(() => {
    ...
    if (location.state?.returnData) {
      ...
      if (location.state.returnData.party) {
        fetchPartyDetails(location.state.returnData.party);
      }
    } else {
      fetchReturnDetails();
    }
  ...
  const fetchReturnDetails = async () => {
    try {
      ...
      setSalesReturnNo(data.salesreturn_no);
  ...
  const fetchInvoiceData = async () => {
    try {
      console.log('roi', invoiceId);
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/sales/invoices/${invoiceId}/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      }); // ✅ API URL yahan daalein
      if (!response.ok) {
        throw new Error('Failed to fetch invoice data');
      }
      const data = await response.json();
      setInvoice(data); // ✅ Invoice state update karega
    } catch (err) {
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/sales/salesreturn/${id}/`, {
        ...
        body: JSON.stringify(returnData), // Corrected data object
      });
  ...
  const handleSubmit = async () => {
    ...
    const returnData = {
      salesreturn_no: salesReturnNo,
      invoice_id: invoiceId,
      date: formatDate(returnDate),
      payment_term: paymentTerms,
      // due_date:dueDate,
      salesreturn_items: formattedItems, // Array of items
      // taxableAmount,
      ...
      if (response.ok) {
        ...
        /*  if (amountReceived > 0) {
          ...
          const paymentPayload = {
            ...
            remarks: `Received for Sales Invoice #${salesReturnNo}`,
          ...
          <div className="flex flex-col-reverse md:flex-row-reverse justify-between items-start gap-6 mb-12">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <input
                  type="text"
                  value={returnData.salesreturn_no}
                  readOnly
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-black dark:text-white p-2 rounded">
                  {loading
                    ? 'Loading...'
                    : error
                    ? 'Error fetching data'
                    : `Invoice No: ${invoice.invoice_no} (Balance: ₹${invoice.total_amount})`}
                </span>
              </div>
            </div>
          </div>
