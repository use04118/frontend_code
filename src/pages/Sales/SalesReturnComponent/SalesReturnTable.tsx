...
const SalesReturnTable: React.FC = () => {
  const API_URL = import.meta.env.VITE_API_URL as string;
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [dateRange, setDateRange] = useState<string>('Last 365 Days');
  const [customStartDate, setCustomStartDate] = useState<Date | null>(null);
  const [customEndDate, setCustomEndDate] = useState<Date | null>(null);
  const [saleReturnData, setSaleReturnData] = useState<SalesReturn[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [partyData, setPartyData] = useState<PartyMap>({});
  ...
  useEffect(() => {
    const fetchReturns = async () => {
      try {
        ...
        if (Array.isArray(data.results)) {
          setSaleReturnData(data.results);
        } else {
    ...
    const fetchParties = async () => {
      try {
        ...
        const data = await response.json();
        if (Array.isArray(data.results)) {
          const partyMap: PartyMap = {};
          data.results.forEach((party: { id: number; party_name: string }) => {
  ...
  const getDateRange = (option: string): [string | null, string | null] => {
    ...
    switch (option) {
      ...
      case 'Yesterday':
        ...
        return [format(yest, 'yyyy-MM-dd'), format(yest, 'yyyy-MM-dd')];
      case 'This Week':
        return [
  ...
  const filteredSalesReturn = saleReturnData.filter((salesReturn) => {
    const invoiceDate = format(new Date(salesReturn.date), 'yyyy-MM-dd');
    ...
    return (
      salesReturn.salesreturn_no.toString().includes(searchTerm) &&
      isWithinDateRange
    );
  });
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold">Sales Return Details</h2>
        ...
        <table className="min-w-full border border-gray-300">
          <thead>
            <tr className="bg-gray-200 text-left dark:bg-gray-600">
              ...
              <th className="border px-4 py-2">Sales Return Number</th>
          ...
          <tbody>
            {filteredSalesReturn.length > 0 ? (
              filteredSalesReturn.map((data) => (
                <tr
                  key={data.salesreturn_no}
                  onClick={() => navigate(`/theme2/${data.id}`)}
                  className="cursor-pointer hover:bg-gray-100"
                >
                  <td className="border px-4 py-2">{data.date}</td>
                  <td className="border px-4 py-2">{data.salesreturn_no}</td>
