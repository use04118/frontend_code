...
const CreateSalesReturnForm = () => {
  ...
  const fetchReturnDetails = async (id) => {
    try {
      ...
      const returnItems = data.invoice_items.map((item) => {
  ...
  useEffect(() => {
    const savedReturnNo = localStorage.getItem('salesReturnNo');
    if (savedReturnNo) {
      setSalesReturnNo(parseInt(savedReturnNo)); // Increment by 1
    }
  }, []);
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/sales/salesreturn/next-number/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_salesreturn_number;
        setSalesReturnNo(nextNumber);
        localStorage.setItem('salesReturnNo', nextNumber.toString());
      }
    } catch (error) {
...
export default CreateSalesReturnForm;
