...
interface ReturnData {
  salesreturn_no: string;
  date: string;
  invoice_no: string;
  party: string;
  salesreturn_items: ReturnItem[];
  total_amount: number;
...
const Theme = () => {
  ...
  useEffect(() => {
    ...
    const fetchReturnDetails = async () => {
      try {
        ...
        if (data) {
          setReturnData(data as ReturnData);
        } else {
          setReturnData(null);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchTcsSetting = async () => {
      try {
        ...
        console.error('Error fetching TCS settings:', error);
      }
    };
  ...
  const generatePDF = () => {
    setTimeout(() => {
      const element = document.getElementById('return-content');
      ...
      const options = {
        margin: [10, 10, 10, 10],
        filename: `${returnData?.salesreturn_no}_SalesReturn.pdf`,
        html2canvas: {
          scale: 5,
          backgroundColor,
          logging: false,
  ...
  const handleEdit = () => {
    navigate(`/edit-salesReturn/${id}`, { state: { returnData } });
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to delete return: ${errorData.message}`);
      }
    } catch (error) {
  ...
  const partyInfo = partyData[returnData.party] || {
    name: 'Loading...',
    mobile: 'Loading...',
    address: 'Loading...',
    state: '',
  ...
  const calculateTaxSummary = (items: ReturnItem[], isSameState: boolean): TaxSummary => {
    ...
    <div className="bg-white dark:bg-gray-900 p-8">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="mt-2 text-right">
              <p style={{ color: themeColors.headerText }}>
                <strong>Return No.:</strong> {returnData.salesreturn_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Invoice No.:</strong> {returnData.invoice_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Date:</strong> {returnData.date}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <tbody>
            {returnData.salesreturn_items.map((item, index) => (
        ...
        >
          ...
          <p className="w-1/5 text-center">{returnData.salesreturn_items.length}</p>
          <p className="w-1/5 text-right">
            ₹{(returnData.total_amount - returnData.taxable_amount).toFixed(2)}
          </p>
          <p className="w-1/5 text-right">₹{returnData.total_amount}</p>
        </div>
        ...
        <div className="flex justify-between text-sm mb-4">
          ...
          >
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Taxable Amount:</p>
              <p>{returnData.taxable_amount}</p>
            </div>
            {(() => {
              const taxSummary = calculateTaxSummary(returnData.salesreturn_items, isSameState);
              ...
              {isTcsEnabled && (
                <div className="flex justify-between mb-1">
                  ...
                  <p className="text-sm font-bold">
                    ₹{returnData.tcs_amount || '-'}
                  </p>
                </div>
              )}
              <div className="flex justify-between">
                <p className="text-sm font-bold">TOTAL AMOUNT</p>
                <p className="text-sm font-bold">₹{returnData.total_amount}</p>
              </div>
            </div>
            {/* Amount Received & Balance */}
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Received Amount:</p>
              <p>₹{returnData.amount_received}</p>
            </div>
            <div className="flex justify-between items-center mb-2 text-right">
              <p className="text-sm font-bold">Balance</p>
              <p className="text-sm font-bold">₹{returnData.balance_amount}</p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <p>{numberToWords(returnData.total_amount)} Rupees</p>
        </div>
      </div>
