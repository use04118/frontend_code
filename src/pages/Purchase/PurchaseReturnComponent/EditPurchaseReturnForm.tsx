...
const EditPurchaseReturnForm = () => {
  const navigate = useNavigate();
  const [purchaseInvoiceNo, setPurchaseReturnNo] = useState(1);
  ...
  useEffect(() => {
    const fetchParties = async () => {
      try {
        const { data } = await api.get('/parties/parties/');
  ...
  useEffect(() => {
    const fetchTcsSetting = async () => {
      try {
        const { data } = await api.get('/sales/settings/tcs-tds/');
  ...
  useEffect(() => {
    const fetchTcsRates = async () => {
      try {
        const { data } = await api.get('/sales/tcs/');
  ...
  useEffect(() => {
    const fetchTdsRates = async () => {
      try {
        const { data } = await api.get('/sales/tds/');
  ...
  const fetchPartyDetails = async (partyId) => {
    try {
      const { data } = await api.get(`/parties/parties/${partyId}/`);
  ...
  const fetchInvoiceDetails = async () => {
    try {
      const { data } = await api.get(`/purchase/purchasereturn/${id}/`);

      setPurchaseReturnNo(data.purchasereturn_no);
      setInvoiceDate(data.date);
      setPaymentTerms(data.payment_term);
      ...
      const invoiceItems = data.purchasereturn_items.map((item) => {
      ...
      console.log(invoiceItems); // Log the data to inspect
    } catch (error) {
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      const response = await api.put(`/purchase/invoices/${id}/`, invoiceData);
  ...
  const fetchTaxRates = async () => {
    try {
      const { data } = await api.get('/inventory/gst-tax-rates/');
  ...
  const handleAddNewTcs = async () => {
    try {
      const { data: newRate } = await api.post('/sales/tcs/', {
        description: taxName,
        section: sectionName,
        rate: rate,
      });
  ...
  const handleAddNewTds = async () => {
    try {
      const { data: newRate } = await api.post('/sales/tds/', {
          ...
          <div className="flex flex-col-reverse md:flex-row-reverse justify-between items-start gap-6 mb-12">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <input
                  type="text"
                  value={invoiceData.purchasereturn_no}
                  readOnly
            ...
            <div className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-lg shadow-lg p-4 relative max-h-[85vh] overflow-y-auto">
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    const res = await api.post('/cash-bank/accounts/', payload);
                    if (res.ok) {
