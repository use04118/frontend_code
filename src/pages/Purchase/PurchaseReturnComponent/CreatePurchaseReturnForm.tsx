...
const CreatePurchaseReturnForm = () => {
  ...
  const [purchaseReturnNo, setPurchaseReturnNo] = useState(1);
    ...
    const fetchParties = async () => {
      try {
        ...
        if (!data.results || !Array.isArray(data.results)) {
          throw new Error('Invalid response format: Expected an array in "results"');
        }

        const validParties = data.results.filter(
          (party: Party) => party && party.party_name,
        );
  ...
  useEffect(() => {
    const fetchTcsSetting = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/sales/settings/tcs-tds/`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    const fetchTcsRates = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/sales/tcs/`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    const fetchTdsRates = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/sales/tds/`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchReturnDetails = async (id) => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await axios.get(`${API_URL}/purchase/purchase/${id}/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      const returnItems = data.purchase_items.map((item) => {
  ...
  const fetchAllInvoices = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/purchase/purchase`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
      ...
      setAllInvoices(response.data.results || []);
    } catch (error) {
  ...
  const fetchItems = async () => {
    try {
      ...
      const allItems = [
        ...itemsResponse.data.results.map((item) => ({ ...item, type: 'item' })),
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/purchase/purchasereturn/next-number/`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
      ...
      const nextNumber = response.data.next_purchase_return_number;
      setPurchaseReturnNo(nextNumber);
      localStorage.setItem('purchaseReturnNo', nextNumber.toString());
    } catch (error) {
  ...
  const handleSaveReturn = async () => {
    if (!selectedParty) {
      alert('Please select a party before saving the return!');
      return;
    }
    ...
    const returnData = {
      purchasereturn_no: purchaseReturnNo,
      date: formatDate(returnDate),
      payment_term: paymentTerms,
      purchase_id: selectedInvoice,
      purchasereturn_items: formattedItems,
      discount: totalDiscount,
      amount_received: amountReceived,
      payment_method: selectedPaymentMode,
      bank_account: selectedPaymentMode !== 'Cash' ? selectedBankAccountId : null,
      is_fully_paid: isFullyPaid,
      notes,
      signature: signatureUrl,
      party: selectedParty.id,
      apply_tcs: applyTcs,
      tcs: applyTcs ? selectedTcsRate?.id : null,
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/purchase/purchasereturn/`,
        returnData,
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <>
      <Breadcrumb pageName="Purchase Returns" />
                ...
                >
                  ...
                  {filteredInvoices.map((invoice) => (
                    <option
                      key={invoice.id}
                      value={invoice.id}
                    >{`Invoice No :${invoice.purchase_no} (Balance: ${invoice.total_amount})`}</option>
                  ))}
                </select>
              </div>
            </div>
