...
const PurchaseReturnTable: React.FC = () => {
  const API_URL = import.meta.env.VITE_API_URL;
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [dateRange, setDateRange] = useState<string>('Last 365 Days');
  const [customStartDate, setCustomStartDate] = useState<Date | null>(null);
  const [customEndDate, setCustomEndDate] = useState<Date | null>(null);
  const [purchaseReturnData, setPurchaseReturnData] = useState<PurchaseReturn[]>([]);
  ...
  useEffect(() => {
    const fetchReturns = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/purchase/purchasereturn/`, {
          headers: {
        ...
        if (Array.isArray(response.data.results)) {
          setPurchaseReturnData(response.data.results);
        } else {
    ...
    const fetchParties = async () => {
      try {
        ...
        if (Array.isArray(response.data.results)) {
          ...
          response.data.results.forEach((party: { id: number; party_name: string }) => {
  ...
  const filteredPurchaseReturn = purchaseReturnData.filter((purchaseReturn) => {
    const invoiceDate = format(new Date(purchaseReturn.date), 'yyyy-MM-dd');
    ...
    return (
      purchaseReturn.purchasereturn_no.toString().includes(searchTerm) &&
      isWithinDateRange
    );
  });
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold">Purchase Return Details</h2>
        ...
        <div className="relative w-1/3">
          ...
          <input
            type="text"
            placeholder="Search by Purchase Return Number..."
        ...
        <table className="min-w-full border border-gray-300">
          <thead>
            <tr className="bg-gray-200 text-left dark:bg-gray-600">
              ...
              <th className="border px-4 py-2">Purchase Return Number</th>
          ...
          <tbody>
            {filteredPurchaseReturn.length > 0 ? (
              filteredPurchaseReturn.map((data) => (
                <tr
                  key={data.purchasereturn_no}
                  onClick={() => {
                    console.log("Navigating to:", data.id);
                    navigate(`/purchasetheme2/${data.id}`);
                  }}
                  className="cursor-pointer hover:bg-gray-100"
                >
                  <td className="border px-4 py-2">{data.date}</td>
                  <td className="border px-4 py-2">{data.purchasereturn_no}</td>
