...
// Add ThemeColors interface at the top
...
// Update the invoiceThemes object with all themes
...
const PurchaseTheme1 = () => {
  const { id } = useParams(); // Get invoice number from URL
  console.log('Invoice Number from URL:', id);
  ...
  // Add new state variables for theme
  ...
  useEffect(() => {
    console.log('Fetching data for invoice_no:', id); // Debugging

    if (!id) return; // Prevent fetching if invoice_no is undefined
    ...
    const fetchInvoiceDetails = async () => {
      try {
        ...
        if (data) {
          ...
          setInvoiceData([]); // Default to empty array if data is not an array
        }
      } catch (error) {
  ...
  const generatePDF = () => {
    try {
      setTimeout(() => {
        ...
        const options = {
          margin: [10, 10, 10, 10],
          filename: `${invoiceData.purchase_no}_Invoice.pdf`,
          html2canvas: {
            scale: 5,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate('/Purchase/Purchase-Invoice'); // Redirect to invoice list page
      } else {
  ...
  const handleEdit = () => {
    navigate(`/edit-purchaseinvoice/${id}`, { state: { invoiceData } });
  };
  ...
  return (
    <div className="p-8" style={{ backgroundColor: themeColors.bodyBg }}>
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="mt-2 text-right">
              <p style={{ color: themeColors.headerText }}>
                <strong>Invoice No.:</strong> {invoiceData.purchase_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
        ...
        >
          ...
          <p className="w-1/5 text-center">{invoiceData.purchase_items.length}</p>
        ...
        <div className="flex justify-between text-sm mb-4">
          ...
          >
            ...
            {(() => {
              ...
              invoiceData.purchase_items.forEach((item) => {
            ...
            <div 
              className="mb-4 border-t border-b pt-2 pb-2 text-right"
              style={{ borderColor: themeColors.tableBorder }}
            >
              {isTcsEnabled && (
                <div className="flex justify-between mb-1">
                  <p className="text-sm font-bold">TCS AMOUNT</p>
                  <p className="text-sm font-bold">₹{invoiceData.tcs_amount}</p>
                </div>
              )}
              {isTdsEnabled && (
                <div className="flex justify-between">
                  <p className="text-sm font-bold">TDS AMOUNT</p>
                  <p className="text-sm font-bold">₹{invoiceData.tds_amount}</p>
                </div>
              )}
              <div className="flex justify-between">
              ...
              {isTdsEnabled && (
                <div className="flex justify-between">
                  ...
                  <p className="text-sm font-bold">₹{invoiceData.total_payable_amount}</p>
                </div>
              )}
            </div>
