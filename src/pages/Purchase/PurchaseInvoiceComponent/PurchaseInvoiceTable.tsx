...
const PurchaseInvoiceTable: React.FC = () => {
  ...
  const [saleInvoiceData, setSaleInvoiceData] = useState<PurchaseInvoiceData[]>([]);
    ...
    const fetchInvoices = async () => {
      try {
        ...
        if (Array.isArray(data.results)) {
          setSaleInvoiceData(data.results);
        } else {
  ...
  const filteredPurchaseInvoice = saleInvoiceData.filter((purchaseInvoice) => {
    const invoiceDate = format(new Date(purchaseInvoice.date), 'yyyy-MM-dd');

    const isWithinDateRange =
      startDate && endDate ? invoiceDate >= startDate && invoiceDate <= endDate : true;

    return (
      purchaseInvoice.purchase_no.toString().includes(searchTerm) && isWithinDateRange
    );
  });
        ...
        <table className="min-w-full border border-gray-300">
          <thead>
            <tr className="bg-gray-200 text-left dark:bg-gray-600 ">
              ...
              <th className="border px-4 py-2">Purchase Invoice Number</th>
          ...
          <tbody>
            {filteredPurchaseInvoice.length > 0 ? (
              filteredPurchaseInvoice.map((data) => (
                <tr
                  key={data.purchase_no}
                  onClick={() => {
                    console.log("Navigating to:", data.id);
                    navigate(`/purchasetheme1/${data.id}`);
                  }}
                  className="cursor-pointer hover:bg-gray-100"
                >
                  <td className="border px-4 py-2">{data.date}</td>
                  <td className="border px-4 py-2">{data.purchase_no}</td>
