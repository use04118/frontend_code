...
const EditPurchaseInvoiceForm = () => {
  ...
  const [purchaseInvoiceNo, setPurchaseInvoiceNo] = useState(1);
  ...
  const fetchInvoiceDetails = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/purchase/purchase/${id}/`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      const invoiceDate = data.date; // Assuming date is in 'YYYY-MM-DD' format from backend
      ...
      setPurchaseInvoiceNo(data.purchase_no)
      ...
      // Assuming the data includes 'invoice_items' array
      const invoiceItems = data.purchase_items.map((item) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (location.state?.invoiceData) {
        ...
        if (location.state.invoiceData.party) {
          await fetchPartyDetails(location.state.invoiceData.party);
        }
      }
    ...
    loadData();
  }, [id, tcsRates.length, tdsRates.length]); // ⛏️ Re-run if `tcsRates` load late
  ...
  // useEffect(() => {
  //   const savedInvoiceNo = localStorage.getItem('purchaseInvoiceNo');
  //   if (savedInvoiceNo) {
  //     setPurchaseInvoiceNo(parseInt(savedInvoiceNo)); // Increment by 1
  //   }
  // }, []);
  ...
  const handleSubmit = async () => {
    ...
    const invoiceData = {
      ...
      // due_date:dueDate,
      purchase_items: formattedItems, // Array of items
      // taxableAmount,
