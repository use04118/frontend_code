...
const CreatePurchaseInvoiceForm = () => {
  ...
  const [purchasesInvoiceNo, setPurchasesInvoiceNo] = useState(1);
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/purchase/purchase/next-number/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_purchase_number;
        setPurchasesInvoiceNo(nextNumber);
        localStorage.setItem('purchaseInvoiceNo', nextNumber.toString());
      }
    } catch (error) {
  ...
  const handleSaveInvoice = async () => {
    ...
    const invoiceData = {
      purchase_no: purchasesInvoiceNo,
      ...
      apply_tds: applyTds,
      tds: applyTcs ? selectedTdsRate?.id : null, // send only if applyTcs is true
    };
  ...
  return (
    <>
      <Breadcrumb pageName="Purchases Invoices" />
