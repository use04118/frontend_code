...
const EditPurchaseOrderForm = () => {
  ...
  const [purchaseOrderNo, setPurchaseOrderNo] = useState(1);
  ...
  useEffect(() => {
    ...
    if (location.state?.orderData) {
      ...
      if (location.state.orderData.party) {
        fetchPartyDetails(location.state.orderData.party);
      }
    } else {
      fetchOrderDetails();
    }
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const orderDate = data.date; // Assuming date is in 'YYYY-MM-DD' format from backend
      ...
      setPurchaseOrderNo(data.purchase_order_no)
      ...
      // Assuming the data includes 'order_items' array
      const orderItems = data.purchaseorder_items.map((item) => {
      ...
      console.log(orderItems); // Log the data to inspect
    } catch (error) {
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/purchase/orders/${id}/`, {
        ...
        body: JSON.stringify(orderData), // Corrected data object
      });
  ...
  // useEffect(() => {
  //   const savedOrderNo = localStorage.getItem('purchaseOrderNo');
  //   if (savedOrderNo) {
  //     setPurchaseOrderNo(parseInt(savedOrderNo)); // Increment by 1
  //   }
  // }, []);
          ...
          <div className="flex flex-col-reverse md:flex-row-reverse justify-between items-start gap-6 mb-12">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <input
                  type="text"
                  value={orderData.purchase_order_no}
                  readOnly
