...
import { FaEdit, FaTrash, FaPalette, FaUndo } from 'react-icons/fa';
...
const PurchaseTheme4 = () => {
  const { id } = useParams(); // Get order number from URL
  console.log('Order Number from URL:', id);
  ...
  useEffect(() => {
    console.log('Fetching data for order_no:', id); // Debugging

    if (!id) return; // Prevent fetching if order_no is undefined

    const fetchOrderDetails = async () => {
      try {
        const token = localStorage.getItem('accessToken');
        const response = await fetch(
          `${API_URL}/purchase/purchaseorder/${id}/`,
          {
            method: 'GET',
            headers: {
        ...
        if (data) {
          ...
          setOrderData([]); // Default to empty array if data is not an array
        }
      } catch (error) {
  ...
  const generatePDF = () => {
    try {
      setTimeout(() => {
        ...
        if (!element) {
          throw new Error('Order content element not found');
        }

        const backgroundColor = themeColors.bodyBg;
        const textColor = themeColors.bodyText;

        const options = {
          margin: [10, 10, 10, 10],
          filename: `${orderData.purchase_order_no}_Order.pdf`,
          html2canvas: {
            scale: 5,
  ...
  const partyInfo = partyData[orderData.party] || {
    name: 'Loading...',
    mobile: 'Loading...',
    address: 'Loading...',
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/purchase/purchaseorder/${id}/`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        navigate('/Purchase/Purchase-Orders'); // Redirect to order list page
      } else {
  ...
  const handleEdit = () => {
    navigate(`/edit-purchaseorder/${id}`, { state: { orderData } });
  };

  const handleConvertToInvoice = async () => {
    const confirmConvert = window.confirm(
      'Are you sure you want to convert this purchase order to an invoice?',
    );
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(
        `${API_URL}/purchase/convert-purchaseorder-to-invoice/${id}/`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        alert('Purcahse Order successfully converted to invoice!');
        navigate('/Purchase/Purchase-Invoice'); // Redirect to invoices page
      } else {
        const errorData = await response.json();
        alert(`Failed to convert: ${errorData.message}`);
      }
    } catch (error) {
    ...
    <div className="bg-white p-8">
      ...
      {showThemeSelector && (
        <>
          ...
          <div style={themeSelectorStyles} ref={themeSelectorRef}>
            ...
            <div style={themeGridStyles} className="theme-grid">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-end">
            ...
            <div className="mt-2 text-right">
              <p style={{ color: themeColors.headerText }}>
                <strong>Order No.:</strong> {orderData.purchase_order_no}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Order Date:</strong> {orderData.date}
              </p>
              <p style={{ color: themeColors.headerText }}>
                <strong>Due Date:</strong> {orderData.due_date}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <p className="w-1/5 text-center">{orderData.purchaseorder_items.length}</p>
          <p className="w-1/5 text-right">
            ₹{(orderData.total_amount - orderData.taxable_amount).toFixed(2)}
          </p>
          <p className="w-1/5 text-right">₹{orderData.total_amount}</p>
        </div>
        ...
        <div className="flex justify-between text-sm mb-4">
          ...
          >
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Taxable Amount:</p>
              <p>{orderData.taxable_amount}</p>
            </div>
            {/* Grouping CGST, SGST, and CESS Rates */}
            {(() => {
              const taxSummary = {};

              orderData.purchaseorder_items.forEach((item) => {
            ...
            <div 
              className="flex justify-between items-center mb-4 border-t border-b pt-2 pb-2 text-right"
              style={{ borderColor: themeColors.tableBorder }}
            >
              <p className="text-sm font-bold">TOTAL AMOUNT</p>
              <p className="text-sm font-bold">₹{orderData.total_amount}</p>
            </div>
            {/* Amount Received & Balance */}
            <div className="flex justify-between items-center mb-2 text-right">
              <p>Received Amount:</p>
              <p>₹{orderData.amount_received}</p>
            </div>
            <div className="flex justify-between items-center mb-2 text-right">
              <p className="text-sm font-bold">Balance</p>
              <p className="text-sm font-bold">₹{orderData.balance_amount}</p>
            </div>
          </div>
        </div>

        {/* Amount in Words */}
        <div className="text-right text-sm" style={{ color: themeColors.bodyText }}>
          <p className="font-bold">Total Amount (in words)</p>
          <p>{numberToWords(orderData.total_amount)} Rupees</p>
        </div>
      </div>
