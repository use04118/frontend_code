...
import UplaodSignature from './UploadSignature';
...
const EditDebitNoteForm = () => {
  ...
  const [debitNoteNo, setDebitNoteNo] = useState(1);
  ...
  const [tdsTaxName, setTdsTaxName] = useState('');
  const [tdsSectionName, setTdsSectionName] = useState('');
  const [tdsRateInput, setTdsRateInput] = useState(0);
  const [selectedTcsRate, setSelectedTcsRate] = useState(null);
  const [selectedTdsRate, setSelectedTdsRate] = useState(null);
  ...
  const [debitNoteData, setDebitNoteData] = useState<{
    ...
    debitnote_items: {
      ...
      amount: number;
      unit_price: number;
      purchasePrice_with_tax?: number; // ✅ Mark as optional
      tax_rate: number;
      gstTaxRat: number;
    }[];
    due_date: string;
  ...
  useEffect(() => {
    ...
    if (location.state?.debitNoteData) {
      setDebitNoteData(location.state.debitNoteData);
      if (location.state.debitNoteData.party) {
        fetchPartyDetails(location.state.debitNoteData.party);
      }
    } else {
      fetchDebitNoteDetails();
    }
  }, [id, tcsRates.length, tdsRates.length]);
  ...
  const fetchDebitNoteDetails = async () => {
    try {
      ...
      const debitNoteDate = data.date; // Assuming date is in 'YYYY-MM-DD' format from backend
      ...
      setDebitNoteNo(data.debitnote_no);
      ...
      const updatedDueDate = new Date(debitNoteDate);
      ...
      if (data.tds && data.apply_tds) {
        setApplyTds(true);

        // Find matching TCS rate object from tcsRates
        const matchedRate = tdsRates.find((rate) => rate.id === data.tds);
        if (matchedRate) {
          setSelectedTdsRate(matchedRate);
        }

        // If tcs_basis (e.g., 'total' or 'taxable') is coming from backend
      }

      // Assuming the data includes 'debitNote_items' array
      const debitNoteItems = data.debitnote_items.map((item) => {
        const quantity = parseFloat(item.quantity) || 1;
        const price = parseFloat(item.purchasePrice_without_tax) || 0;
      ...
      setSelectedItems(debitNoteItems);
      console.log(debitNoteItems); // Log the data to inspect
    } catch (error) {
  ...
  const handleSubmit2 = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/purchase/debitnote/${id}/`, {
        ...
        body: JSON.stringify(debitNoteData), // Corrected data object
      });

      if (response.ok) {
        alert('DebitNote updated successfully!');
        navigate(`/Sales/Sales-DebitNote`);
      } else {
        alert('Failed to update debitNote.');
      }
    } catch (error) {
  ...
  const balanceAmount = isFullyPaid
    ? 0
    : totalAmount - amountReceived - tdsAmount;
  ...
  useEffect(() => {
    if (!applyTds || !selectedTdsRate?.rate) {
      setTdsAmount(0);
      return;
    }

    const calculatedTds = (taxableAmount * selectedTdsRate.rate) / 100;
    setTdsAmount(calculatedTds);
  }, [applyTds, selectedTdsRate, taxableAmount]);
  ...
  useEffect(() => {
    ...
    /*  if (applyTds && selectedTdsRate?.rate) {
      total -= (taxableAmount * selectedTdsRate.rate) / 100;
    } */
      ...
      prevItems.map((item) =>
        item.uid === uid
          ? {
              ...
              amount:
                newQuantity * item.purchasePrice_with_tax -
                (newQuantity * item.purchasePrice_with_tax * item.discount) /
                  100,
            }
          : item,
      ),
  ...
  const handleSelectItem = (item) => {
    ...
    const price = parseFloat(
      item.purchasePrice_without_tax || item.purchasePrice || 0,
    );
    const discount = parseFloat(item.discount || 0);
    ...
    const amount = parseFloat(
      item.purchasePrice_with_tax || item.purchasePrice || 0,
    );
  ...
  const handleCheckboxChange = () => {
    if (!isFullyPaid) {
      const payableAmount = applyTds ? totalAmount - tdsAmount : totalAmount;
      setAmountReceived(Number(payableAmount.toFixed(2))); // ✅ Round to 2 decimal places
    } else {
      setAmountReceived(0);
    }
  ...
  // useEffect(() => {
  //   const savedDebitNoteNo = localStorage.getItem('debitNoteNo');
  //   if (savedDebitNoteNo) {
  //     setDebitNoteNo(parseInt(savedDebitNoteNo)); // Increment by 1
  //   }
  // }, []);

  const handleSubmit = async () => {
    if (!selectedParty) {
      alert('Please select a party before saving the debitNote!');
      return;
    }
    ...
    const debitNoteData = {
      debitnote_no: debitNoteNo,
      purchasereturn_id: parseInt(selectedInvoice),
      date: formatDate(debitNoteDate),
      payment_term: paymentTerms,
      // due_date:dueDate,
      debitnote_items: formattedItems, // Array of items
      // taxableAmount,
      ...
      apply_tds: applyTds,
      tds: applyTcs ? selectedTdsRate?.id : null,
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('DebitNote updated Successfully!');
        console.log(result);
        navigate(`/Sales/Debit-Note`);
      } else {
        alert('Failed to save debitnote');
      }
    } catch (error) {
      console.error('Error saving debitnote:', error);
    ...
    <>
      ...
      <div className="bg-gray-100 dark:bg-gray-900 p-4">
        <div className="max-w-7xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
          <div className="flex flex-row-reverse justify-between items-start mb-40">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <input
                  type="text"
                  value={debitNoteData.debitnote_no}
                  readOnly
                      ...
                      {applyTds && (
                        <div className="flex items-center space-x-2">
                          ...
                          <select
                            ...
                            value={
                              selectedTdsRate
                                ? JSON.stringify(selectedTdsRate)
                                : ''
                            }
                            onChange={(e) => {
                              if (e.target.value === 'add_new') {
                                ...
                                setSelectedTdsRate(JSON.parse(e.target.value));
                              }
                            }}
                          >
                            <option value="" disabled hidden>
          ...
          <div className="flex justify-start gap-4">
            ...
            <button
              ...
              disabled={isSaving}
            >
              {isSaving ? 'Saving...' : 'Save DebitNote'}
            </button>
          </div>
        </div>
        {showAddTcsModal && (
        ...
        {showAddTdsModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white p-6 rounded-lg shadow-md w-[90%] max-w-md relative">
              <h2 className="text-lg font-bold mb-4">Add TDS Rate</h2>
              <button
                className="absolute top-2 right-2 text-gray-500 hover:text-gray-800"
              ...
              <div className="flex flex-col gap-3">
                <input
                  ...
                  className="border px-3 py-2 rounded"
                  value={tdsTaxName}
                  onChange={(e) => setTdsTaxName(e.target.value)}
                />
                <input
                  type="text"
                  placeholder="Enter Section Name"
                  className="border px-3 py-2 rounded"
                  value={tdsSectionName}
                  onChange={(e) => setTdsSectionName(e.target.value)}
                />
                <input
                  type="number"
                  placeholder="Enter Rate (%)"
              ...
              <div className="flex justify-end mt-4">
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      const response = await fetch(`${API_URL}/sales/tds/`, {
                        ...
                        body: JSON.stringify({
                          description: tdsTaxName,
                          section: tdsSectionName,
                          rate: tdsRateInput,
                        }),
                      });
                  ...
                  className="px-4 py-2 bg-purple-600 text-white rounded disabled:opacity-50"
                  disabled={!tdsTaxName || !tdsSectionName || tdsRateInput <= 0}
                >
                  Save
                </button>
              </div>
