...
const CreateDebitNoteForm = () => {
  ...
  const fetchReturnDetails = async (id) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/purchase/purchasereturn/${id}/`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const returnItems = data.purchasereturn_items.map((item) => {
  ...
  const fetchAllInvoices = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/purchase/purchasereturn`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const fetchNextInvoiceNumber = async () => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(
        `${API_URL}/purchase/debitnote/next-number/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        const nextNumber = data.next_purchase_debit_number;
        setDebitNoteNo(nextNumber);
        localStorage.setItem('debitNoteNo', nextNumber.toString());
      }
    } catch (error) {
  ...
  const handleSaveDebitNote = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      if (!token) throw new Error('Token not found');
      const response = await fetch(`${API_URL}/purchase/debitnote/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        alert('DebitNote Saved Successfully!');
        console.log(result);
        fetchNextInvoiceNumber();
  ...
  return (
    <>
      <Breadcrumb pageName="Debit Notess" />
                ...
                >
                  ...
                  {filteredInvoices.map((invoice) => (
                    <option
                      key={invoice.id}
                      value={invoice.id}
                    >{`Sales Return No :${invoice.purchasereturn_no} (Balance: ${invoice.total_amount})`}</option>
                  ))}
                </select>
              </div>
            </div>
...
export default CreateDebitNoteForm;
