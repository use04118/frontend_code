...
import CreatePurchaseInvoice from './PurchaseInvoiceComponent/CreatePurchaseInvoice';
import PurchaseInvoiceTable from './PurchaseInvoiceComponent/PurchaseInvoiceTable';
...
const PurchaseInvoice: React.FC = () => {
  ...
  const fetchPaid = async () => {
    try {
      ...
      const response = await axios.get<{ totalPaid: number }>(
        `${API_URL}/purchase/purchase/paid/`,
        {
          headers: {
  ...
  const fetchUnpaid = async () => {
    try {
      ...
      const response = await axios.get<{ totalUnPaid: number }>(
        `${API_URL}/purchase/purchase/unpaid/`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
  ...
  const totalPurchases = (paid ?? 0) + (unpaid ?? 0);

  return (
    <>
      <h4 className="text-2xl font-bold mb-4">Purchase Invoices</h4>
